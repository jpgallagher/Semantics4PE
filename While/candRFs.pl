...
test1(RF) :-
	go(p(A,B,C,D),[1*A+ -1*B>=2,1*A+ -1*B+ -1*C>=1,1*B> -1,1*A+ -1*D>=2,1*C>0,1*D> -1,1*B+ -1*E= -1,1*C+ -1*F=1,1*D+ -1*G= -1,A1=A],p(A1,E,F,G),RF).
	
test2(RF) :-
	go(p(A,B,C,D),[1*A+ -1*D>=2,1*A+ -1*B>=1,1*A+ -1*B+ -1*C>=1,1*B> -1,1*C> -1,1*D> -1,1*D+ -1*E= -1,A1=A, B1=B, C1=C],p(A1,B1,C1,E),RF).
	
test3(RF) :-
	go(p(A,B,C,D),[1*A+ -1*B+ -1*C+1*D>=0,1*A+ -1*B+ -1*C>=0,1*B> -1,1*C>=1,1*D> -1,1*B+ -1*E= -1,1*C+ -1*F=1, A1 = A, D1 = D],p(A1,E,F,D1),RF).
	
test4(RF) :-
	go(p(B),[B < 100, A = B+1],p(A),RF).

test5(RF) :-
	go(p(A,B,C,D),[-1*D+1*H>=1,1*D+ -1*H>= -2,1*C>=1,1*B>=1,1*D>=0,1*B+1*D+ -1*F+ -1*H= -1,1*C+ -2*D+ -1*G+2*H=3,1*A+ -1*E=0],p(E,F,G,H),RF).
	
test6(RF) :-
	go(p(A,B),[A>0,B>0,A1=A+1,B1=B-1],p(A1,B1),RF).	
	
test7(RF) :-
	go(p(A,B,C,D,E,F,G,H),[1*I+ -1*J>=1,1*I>=0,1*I+ -1*A=0,1*J+ -1*C=0,1*P+ -1*H=0,1*O+ -1*G=0,1*N+ -1*F=0,1*L=0,1*J+ -1*K=0,1*M+ -1*E=0,1*J+ -1*B= -1,1*D=1],p(I,J,K,L,M,N,O,P),RF).
